...
Процесс абсолютно тот же. Главное, чтобы на удаленном сервере (`192.168.0.10`) порт контейнера был опубликован на хост с помощью опции `ports` в `docker-compose.yml` или флага `-p` в `docker run`.

_Пример `docker-compose.yml` для удаленного сервера:_
