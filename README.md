...
2.  **Запустите `add` скрипт:**
    ...
    - `Введите ваш email`: `admin@example.com`
    - `Куда проксировать трафик? [1-2]`: `2` (на порт хост-машины)
    - `Введите порт на хост-машине`: `8080`
    - `Использовать staging-сервер? (y/n)`: `n`

Готово! Шлюз будет проксировать трафик с `https://legacy.example.com` на `localhost:8080` вашей хост-машины. Это возможно благодаря специальному DNS-имени `host.docker.internal`, которое Docker предоставляет для доступа к хосту изнутри контейнера.
